...
const questions = 
[
  new Question("Выберите подходящий вариант использования визуальных средств", 
  [
    new Answer("Забыть о композиции кадра и использовать случайные углы съемки", 0),
    new Answer("Позволить оператору камеры делать съемки по своему усмотрению", 0),
    new Answer("Использовать один и тот же стандартный вид съемки для всех сцен", 0),
    new Answer("Использовать визуальные техники, такие как цветовая палитра и освещение, чтобы подчеркнуть эмоциональное содержание", 1)
  ]),
...
function Update()
{
  ...
  nextButton.addEventListener("click", function () {
    ...
    pagesElem.innerHTML = (9) + " / " + quiz.questions.length;
